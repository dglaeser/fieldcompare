...
def mesh_equal(source: MeshInterface,
               target: MeshInterface,
               rel_tol: float = _default_base_tolerance(),
               abs_tol: float = _default_base_tolerance()) -> PredicateResult:
    ...
    for cell_type in source.cell_types:
        if len(source.connectivity(cell_type)) != len(target.connectivity(cell_type)):
            return PredicateResult(
                False,
                report=f"Differing number of cells of type '{cell_type}'"
            )
        if not all(
            ExactEquality()(sorted(source_corners), sorted(target_corners))
            for source_corners, target_corners in zip(
                source.connectivity(cell_type),
                target.connectivity(cell_type)
            )
        ):
            return PredicateResult(
                False,
                report=f"Differing connectivity detected for cell of type '{cell_type}'"
            )
