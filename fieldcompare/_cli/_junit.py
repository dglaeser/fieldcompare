...
def as_junit_xml_element(suite: TestSuite, timestamp: str) -> Element:
    ...
    xml_tree.set("errors", str(sum(1 for t in suite if t.result == TestResult.error)))
    xml_tree.set("failures", str(sum(1 for t in suite if t.result == TestResult.failed)))
    xml_tree.set("skipped", str(sum(1 for t in suite if t.result == TestResult.skipped)))
...
def _add_test_case(test_suite: Element, test: Test) -> None:
    testcase = SubElement(test_suite, "testcase")
    testcase.set("name", test.name)
    testcase.set("classname", "n/a")
    testcase.set("status", str(test.result).replace("TestResult.", ""))
